...
import { getSignedBioAuth, cacheBioAuth, registerAccount } from "~/mina.server";
...
export async function loader({ request, params }: LoaderArgs) {
  ...
  const thisRoute = `/mina/register/${params.id}`;

  if (isAuthed) {
    // if authenticated, get the signed bioAuth and cache it
    const auth = await requireAuthenticatedUser(request);

    await registerAccount(auth.id, params.id);
      ...
      {bioAuth && (
        <>
          <div className="flex text-center">
            Your crypto-biometric authorization of the account has been
            registered.
            <br />
